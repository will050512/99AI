...
let OpenAIDrawService = OpenAIDrawService_1 = class OpenAIDrawService {
    ...
    async dalleDraw(inputs, buildMessageFromParentMessageId) {
        var _a, _b, _c, _d;
        common_1.Logger.log('開始遞交 Dalle 繪圖任務 ', 'DrawService');
        ...
        if (isDalleChat === '1') {
            try {
                common_1.Logger.log('已開啟連續繪畫模式', 'DalleDraw');
        ...
        try {
            ...
            try {
                revised_prompt_cn = await this.openAIChatService.chatFree(`根據提示詞{${response.data.data[0].revised_prompt}}, 模擬AI繪畫機器人的語氣，用中文回覆，告訴用戶已經畫好了`);
            }
            catch (error) {
